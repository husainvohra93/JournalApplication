...
public class UserScheduler {
    ...
    public void fetUsersAndSendSaMail(){
        ...
        for (UserEntity user : usersForSA){
            ...
            List<Sentiment> sentiments = journalEntries.stream().filter(x->x.getDate().isAfter(LocalDateTime.now().minus(10, ChronoUnit.DAYS))).map(x->x.getSentiment()).collect(Collectors.toList());
            ...
            if(mostFrequentSentiment != null) {
                ...
                try{
                    kafkaTemplate.send("husainTopic", sentimentData.getEmail(), sentimentData);
                    log.info("Sent message to Topic successfully");
                }catch (Exception e){
